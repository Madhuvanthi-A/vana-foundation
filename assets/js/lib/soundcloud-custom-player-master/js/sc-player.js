...
(function($) {
  ...
  var apiKey,
      ...
      onVolume = function(volume) {
        console.log(volume, audioEngine);
        ...
        audioEngine.toggleSound(soundVolume);
      },
      positionPoll;
  ...
  var scrub = function(node, xPos) {
    var $scrubber = $(node).closest('.sc-time-span'),
        ...
        $played = $scrubber.closest('.sc-played'),
        //relative = Math.min($buffer.width(), (xPos  - $available.offset().left) / $available.width()) ;
        relative = Math.min($buffer.width(), (xPos  - $buffer.offset().left) / 227);
